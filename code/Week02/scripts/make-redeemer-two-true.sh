...
# Build redeemer-two-true address
cardano-cli address build \
    --payment-script-file "$assets/redeemer-two-true.plutus" \
    --testnet-magic 2 \
    --out-file "$assets/redeemer-two-true.addr"
...
cardano-cli transaction build \
    ...
    --testnet-magic 2 \
    --tx-in "$txin" \
    --tx-out "$(cat "$assets/redeemer-two-true.addr") + 3000000 lovelace" \
